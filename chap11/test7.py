...
class Seq2SeqEncoder(d2l.Encoder):
    ...
    def forward(self, X, *args):
        ...
        # 移除 *args 参数，因为 d2l 的标准接口不传递 enc_valid_lens
        embs = self.embedding(d2l.astype(d2l.transpose(X), d2l.int64))
        # embs shape: (num_steps, batch_size, embed_size)
        outputs, state = self.rnn(embs)
        # outputs shape: (num_steps, batch_size, num_hiddens)
        # state shape: (num_layers, batch_size, num_hiddens)
        return outputs, state  # 移除 enc_valid_lens
...
class Seq2SeqAttentionDecoder(AttentionDecoder):
    ...
    def init_state(self, enc_outputs, enc_valid_lens):
        # 修改：enc_outputs 现在是一个元组 (outputs, state)
    ...
    def forward(self, X, state):
        ...
        # 分离 hidden_state 为 h 和 c
        if isinstance(hidden_state, tuple):
            h, c = hidden_state
        else:
            h, c = hidden_state, None
        ...
        for x in X:
            ...
            if c is not None:
                ...
                out, h = self.rnn(x.permute(1, 0, 2), h)
                c = None
        ...
        if c is not None:
            ...
            return outputs.permute(1, 0, 2), [enc_outputs, h, enc_valid_lens]

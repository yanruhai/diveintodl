...
class MultiHeadAttention(d2l.Module):  #@save
    ...
    def forward(self, queries, keys, values, valid_lens):
        ...
        values = self.transpose_qkv(self.W_v(values))#进入之前(2,6,100),运行之后(10,6,20)
    ...
    def transpose_qkv(self, X):
        ...
        X = X.reshape(X.shape[0], X.shape[1], self.num_heads, -1)#queries变量进来的时候维度是(2,4,100),num_heads=5,-1自动计算维度为100/5=20,转成2,4,5,20
        # Shape of output X: (batch_size, num_heads, no. of queries or key-value
        # pairs, num_hiddens / num_heads)
        X = X.permute(0, 2, 1, 3)#转成(2,5,4,20)
        # Shape of output: (batch_size * num_heads, no. of queries or key-value
        # pairs, num_hiddens / num_heads)
        return X.reshape(-1, X.shape[2], X.shape[3])#返回10,4,20维度,保持最后两维

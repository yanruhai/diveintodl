...
encoder = torch.jit.script(Seq2SeqEncoder(
    len(data.src_vocab), embed_size, num_hiddens, num_layers, dropout))
decoder = torch.jit.script(Seq2SeqDecoder(
    len(data.tgt_vocab), embed_size, num_hiddens, num_layers, dropout))
model = torch.jit.script(Seq2Seq(encoder, decoder, tgt_pad=data.tgt_vocab['<pad>'],
                lr=0.005))

...
# 2. 定义回归模型（三层全连接神经网络）
class RegressionModelA(nn.Module):
    def __init__(self, input_size):
        super(RegressionModelA, self).__init__()  # 继承nn.Module的初始化方法
...
class RegressionModelB(nn.Module):
    def __init__(self):
        super(RegressionModelB, self).__init__()  # 继承nn.Module的初始化方法
        # 定义神经网络层：用nn.Sequential按顺序包装层（前向传播时按顺序执行）
        self.layers = nn.Sequential(
            nn.LazyLinear(64),  # 输入层→隐藏层1：输入维度=input_size，输出维度=64
            nn.ReLU(),  # 激活函数：ReLU（解决线性模型的表达能力不足问题）
            nn.Linear(64, 32),  # 隐藏层1→隐藏层2：输入维度=64，输出维度=32
            nn.ReLU(),  # 激活函数：ReLU
        )
...
class RegressionModelC(nn.Module):
    def __init__(self):
        super(RegressionModelC, self).__init__()  # 继承nn.Module的初始化方法
        # 定义神经网络层：用nn.Sequential按顺序包装层（前向传播时按顺序执行）
        self.layers = nn.Sequential(
            nn.LazyLinear(64),  # 输入层→隐藏层1：输入维度=input_size，输出维度=64
            nn.ReLU(),  # 激活函数：ReLU（解决线性模型的表达能力不足问题）
...
model1 = RegressionModelA(input_size)
model2=RegressionModelB()
model3=RegressionModelC()
...
optimizer = optim.Adam(  # 优化器：Adam（自适应学习率优化器，比SGD收敛更快、更稳定）
    [param for model in [model1, model2, model3] for param in model.parameters()],  # 待优化的参数：模型中所有可训练参数（权重、偏置）
    lr=0.001  # 学习率：控制参数更新的步长（0.001是常用初始值）
)

def net_forward(model1,model2,model3,batch_X):
    outputs = model1(batch_X)
    outputs = outputs + model2(outputs)
    outputs = outputs + model3(outputs)
    return outputs
...
for epoch in range(epochs):
    ...
    for batch_X, batch_y in train_loader:
        ...
        outputs = net_forward(model1,model2,model3,batch_X)
        # 2. 计算损失：预测值与真实值的MSE
    ...
    with torch.no_grad():
        ...
        for batch_X, batch_y in test_loader:
            outputs = net_forward(model1, model2, model3, batch_X)  # 前向传播计算预测值
...
model3.eval()
with torch.no_grad():
    y_pred = net_forward(model1, model2, model3, X_test)  # 对整个测试集计算预测值

...
def _download(self):
    fname =  self.root+'//1342.txt.utf-8'#傲慢与偏见
    with open(fname,encoding='utf-8') as f:
        return f.read()
data=d2l.TimeMachine(batch_size=1024, num_steps=32,num_val=80*1024)
trainer.prepare_data(data)
trainer.prepare_model(model)
trainer.model.eval()
ppl_list=[]
x_xi=range(80)
va_iter= iter(trainer.val_dataloader)
for epoch in range(80):
    batch=next(va_iter)
    with torch.no_grad():
        batch=trainer.prepare_batch(batch)
        trainer.prepare_model(model)
        #inputs = [x.float() for x in batch[:-1]]  # 转换每个输入张量为 float32
        l= model.loss(model(*batch[:-1]),batch[-1])
        ppl_list.append(torch.exp(l).detach())
# 创建n行m列的子图，返回（画布对象，轴域数组）
fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(8, 4))
ppl_np = [tensor.cpu().numpy() for tensor in ppl_list]
axes.plot(x_xi,ppl_np)
plt.show()

...
trainer = d2l.Trainer(max_epochs=70, gradient_clip_val=1, num_gpus=1)  # 训练
...
def predict(self, prefix, num_preds, vocab, device=None):
    ...
    for i in range(len(prefix) + num_preds - 1):
        ...
        embs = self.one_hot(X)
        rnn_outputs, state = self.rnn(embs, state)#rnn_outputs长度为1的列表,每个元素1*128
        if i < len(prefix) - 1:  # Warm-up period (this means prediction is in given)
            outputs.append(vocab[prefix[i + 1]])
        else:  # Predict num_preds steps
            Y = self.output_layer(rnn_outputs)#Y= 1*1*28
            Y_pre = F.relu(Y) / F.relu(Y).sum()  # 保持tensor，总和精确1.0
            # 展平为(28,)以采样
            probs = Y_pre.flatten()  # 或 Y_pre.view(-1)
            samp = int(torch.multinomial(probs, num_samples=1).item())#用于从多项分布（multinomial distribution）中采样索引。
            #item()是从张量中取出数据
            outputs.append(samp)
